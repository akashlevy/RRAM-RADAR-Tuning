...
for pw in data.pw.unique():
    ...
    means, stds = means[means != 40772.4958], stds[means != 40772.4958]
    
    pareto = pg.non_dominated_front_2d(points=zip(1/means, stds))
    pmeans, pstds = means[pareto], stds[pareto]
    print zip(pmeans, pstds)
    fit1 = np.polyfit(np.log(pmeans[means < 6e3]), np.log(pstds[means < 6e3]), 1)
    fit2 = np.polyfit(np.log(pmeans[means > 6e3]), np.log(pstds[means > 6e3]), 1)
    ...
    plt.loglog(means[pareto], stds[pareto], '.', color='red', label="Pareto optimal points")
    x = np.exp(np.linspace(np.log(4e3), np.log(6e3)))
    plt.loglog(x, np.exp(fit1[0] * np.log(x) + fit1[1]), '--', color='green', label="Low resistance fit")
    x = np.exp(np.linspace(np.log(6e3), np.log(50e3)))
