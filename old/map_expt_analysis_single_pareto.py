...
for pw in data.pw.unique():
    ...
    pmeans, pstds = means[pareto], stds[pareto]
    print zip(pmeans, pstds)
    fit1 = np.polyfit(np.log(pmeans[means < 7.8e3]), np.log(pstds[means < 7.8e3]), 1)
    fit2 = np.polyfit(np.log(pmeans[means > 7.8e3]), np.log(pstds[means > 7.8e3]), 1)
    ...
    x = np.exp(np.linspace(np.log(4e3), np.log(7.8e3)))
    plt.loglog(x, np.exp(fit1[0] * np.log(x) + fit1[1]), '--', color='0.5', label="Fit line")
    x = np.exp(np.linspace(np.log(7.8e3), np.log(50e3)))
