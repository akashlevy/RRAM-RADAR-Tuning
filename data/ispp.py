...
for step in np.arange(0.01, 0.16, 0.01):
    data = pd.read_csv('ispp-cb-%.2f-4-14-20.csv' % step, delimiter='\t', names=['addr', 'nreads', 'nsets', 'nresets', 'rf', 'if', 'rlo', 'rhi', 'success'], index_col=False)
...
# ISPP Mean Step
grouped = data[data['stepsize'] == 0.03].groupby(['bin'])
npulses = grouped['npulses']
npulses_mean = npulses.mean()
npulses_std = npulses.std()
