...
data = pd.read_csv('data/set-sweep-bl-inner-5-1-20.csv', delimiter='\t', names=['addr', 'pw', 'blv', 'wlv', 'ri', 'rf'])
...
plt.legend(["%.1f" % n for n in np.arange(1.5, 3.5, 0.5)], title='VBL (V)')
plt.tight_layout()
...
#plt.legend(["%.1f" % n for n in np.arange(2.5, 3.1, 0.1)], title='VWL (V)', ncol=2)
plt.tight_layout()
