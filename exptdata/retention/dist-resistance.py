...
for expt, bpc in product(range(1, 7+1), (3,)):
    ...
    for pp, loc in zip(['pre','post'],[211,212]):
        ...
        if pp == 'post':
            ...
            plt.title('Resistance Distributions Pre/Post-Bake')
        ...
        for i in ranges:
            ...
            sns.distplot(rdata['rf'], kde=True, label='Range %d' % i, axlabel=False)
            plt.axvline(Rmins[i], -0.1, 1, color=color, linestyle=':', linewidth=1)
            plt.axvline(Rmaxs[i], -0.1, 1, color=color, linestyle=':', linewidth=1)
        plt.legend(borderpad=0.2, prop={'size': 10})
    plt.gca().set_xscale('log')
    plt.gca().set_yscale('log')
    plt.subplots_adjust(wspace=0, hspace=0)
    plt.tight_layout()
    plt.savefig('figs/dist%d-%dbpc-r.pdf' % (expt, bpc))
    #plt.show()
